...
describe("remote dispatch generation apply contract", () => {
  it("defines typed per-replica generation, settings, release and capability scope", async () => {
    ...
    expect(control).toMatch(/message DispatchApplyRequest\s*\{[^}]*uint64 generation\s*=\s*1;[^}]*string instance_id\s*=\s*2;[^}]*string settings_digest\s*=\s*3;[^}]*string release_digest\s*=\s*4;[^}]*repeated CapabilityDispatchScope capabilities\s*=\s*5;/s);
    expect(control).toMatch(/message CapabilityDispatchScope\s*\{[^}]*string capability\s*=\s*1;[^}]*repeated InvocationMode modes\s*=\s*2;/s);
    expect(control).toMatch(/message DispatchApplyResponse\s*\{[^}]*uint64 generation\s*=\s*1;[^}]*string replica_identity_uri\s*=\s*2;[^}]*string manifest_digest\s*=\s*3;[^}]*string settings_digest\s*=\s*4;[^}]*string release_digest\s*=\s*5;[^}]*string dispatch_digest\s*=\s*6;/s);
  });
